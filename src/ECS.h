...
struct ComponentIdCounter {
  ...
  template <typename T> static std::uint32_t getId() {
...
public:
  ...
  template <typename T> std::size_t getComponentCount() const {
...
private:
  ...
  template <typename T> ComponentPool<T, MAX_ENTITIES> &getPool() const {
